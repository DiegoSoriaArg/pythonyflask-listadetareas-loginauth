...
def index():
    db, c = get_db()
    c.execute(
        'select t.id, t.description, u.username, t.complete, t.created_at '
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'select t.id, t.description, t.complete, t.created_by, t.created_at, u.username ' 
...
def update(id):
    ...
    if request.method == 'POST':
        description = request.form['description']
        completed = True if request.form.get('complete') == 'on' else False
        error = None
        ...
        if error is not None:
            ...
            db, c = get_db()
            c.execute(
                'update todo set description = %s, complete = %s'
